...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if foodLeft <=1 or gameState.getAgentState(self.index).numCarrying > 3:
      bestDist = 9999
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  """
  This agent has trouble running from the opposing pac-man when on opp territory.
  """
...
class DefensiveReflexAgent(CaptureAgent):
	...
	def registerInitialState(self, gameState):
		...
		self.oppAgents = CaptureAgent.getOpponents(self, gameState)
		self.myFood = CaptureAgent.getFood(self, gameState).asList()
		self.oppFood = CaptureAgent.getFoodYouAreDefending(self, gameState).asList()
	...
	def chooseAction(self, gameState):
		...
		distToOps = []
		for opponent in self.oppAgents:
